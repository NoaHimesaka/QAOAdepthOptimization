"""
The PG model with random initial value
"""
...
for jk in range(multi):
    ...
    """
    beta = q.var((0.02 * np.random.rand(step, 1)+0.29 * np.ones((step, 1),dtype='float64')), True)
    gamma = q.var((0.02* np.random.rand(step, 1)+0.29 * np.ones((step, 1),dtype='float64')), True)
    beta_origion=beta
    gamma_origion=gamma
    vqc = q.VariationalQuantumCircuit()
    for i in qlist:
