"""
The APG model with random initial value
"""
...
Parameter setting
"""
multi=100 # the number of experiments
...
def maxi(list,init,end):
    ...
    return temp
loss_cal = np.zeros((multi,iter))
"""
Iteration process
"""
for jk in range(multi):
    """
    Initial value of beta and gamma
    """
    beta = q.var((0.06 * np.random.rand(step, 1)+0.27 * np.ones((step, 1),dtype='float64')), True)
    gamma = q.var((0.06* np.random.rand(step, 1)+0.27 * np.ones((step, 1),dtype='float64')), True)
    beta_origion=beta
    gamma_origion=gamma
    print("origion beta",eval(beta))
    vqc = q.VariationalQuantumCircuit()
    for i in qlist:
        vqc.insert(q.VariationalQuantumGate_H(i))
